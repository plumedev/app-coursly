      ...
      <v-col cols="12" sm="10" md="8" lg="6">
        <v-card class="pa-6" elevation="4" rounded="lg">
          ...
          <div class="text-center mb-6">
            <v-btn :color="isShoppingModeActive ? 'error' : 'success'" size="large"
            ...
            <v-chip v-if="isShoppingModeActive" color="success" size="small" class="ml-2">
              Mode courses actif
            </v-chip>
          </div>
          ...
          <template v-if="!isShoppingModeActive">
            ...
            <v-form @submit.prevent="handleSubmit" ref="formRef">
              <v-row>
                <v-col cols="12" md="5">
                  <v-text-field v-model="form.name" label="Nom du produit" :rules="[rules.required]" variant="outlined"
                ...
                <v-col cols="12" md="4">
                  <v-text-field v-model="form.unit" label="Unité (optionnel)" placeholder="kg, g, L, ml, unité..."
                    variant="outlined" density="comfortable" hint="Ex: kg, g, L, ml, unité" persistent-hint />
                </v-col>
              </v-row>
            ...
            <v-alert v-if="!isLoading && (!items || items.length === 0)" type="info" variant="tonal" class="mb-4">
            ...
            <v-list v-if="items && items.length > 0" lines="two" class="mb-4">
              <v-list-item v-for="item in items" :key="item.id" class="mb-2">
                ...
                <template v-slot:append>
                  <v-btn icon variant="text" size="small" @click="startEdit(item)" :prepend-icon="mdiPencil">
            ...
            <v-btn v-if="selectedItems.length > 0" color="error" variant="outlined" @click="handleDeleteSelected"
              :loading="isDeleting" block class="mb-2" :prepend-icon="mdiDelete">
          ...
          <template v-else>
              ...
              <v-list-item v-for="item in uncheckedItems" :key="item.id" class="mb-2">
                <template v-slot:prepend>
                  <v-checkbox :model-value="item.checked || false"
                    @update:model-value="handleToggleChecked(item.id, $event)" color="success" hide-details />
                </template>
            ...
            <v-list v-if="checkedItems.length > 0" lines="two" class="mb-4">
              <v-list-item v-for="item in checkedItems" :key="item.id" class="mb-2"
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form @submit.prevent="handleUpdate" ref="editFormRef">
            ...
            <v-text-field v-model="editForm.unit" label="Unité (optionnel)" placeholder="kg, g, L, ml, unité..."
              variant="outlined" hint="Ex: kg, g, L, ml, unité" persistent-hint />
          </v-form>
        </v-card-text>
...
import { mdiPlus, mdiPencil, mdiDelete, mdiCart, mdiStop } from '@mdi/js'
